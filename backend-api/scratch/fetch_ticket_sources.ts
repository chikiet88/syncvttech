...
function decompress(data: any): any {
  ...
  try {
    ...
    try { return JSON.parse(zlib.gunzipSync(buf).toString('utf-8')).data || JSON.parse(zlib.gunzipSync(buf).toString('utf-8')); }
    catch { try { return JSON.parse(zlib.inflateSync(buf).toString('utf-8')).data || JSON.parse(zlib.inflateSync(buf).toString('utf-8')); }
    catch { return JSON.parse(zlib.inflateRawSync(buf).toString('utf-8')).data || JSON.parse(zlib.inflateRawSync(buf).toString('utf-8')); } }
...
async function main() {
  ...
  // 3. Fetch /Marketing/TicketSourceList page
  ...
  if (typeof pageRes.data === 'string') {
    await Bun.write('scratch/ticket_source_list.html', pageRes.data);
    console.log('Saved scratch/ticket_source_list.html');

    const $ = cheerio.load(pageRes.data);
    const formToken = $('input[name="__RequestVerificationToken"]').val() as string;
    if (formToken) xsrfToken = formToken;

    console.log('\n--- Script tags in TicketSourceList ---');
    $('script').each((i, el) => {
      const text = $(el).html() || '';
      console.log(`Script #${i} (length ${text.length}):`, text.substring(0, 500));
    });
  }
  ...
  // Test handlers
  const handlersToTest = ['LoadData', 'LoadDataList', 'LoadList', 'LoadSource', 'LoadGroup', 'LoadDetail'];
  for (const h of handlersToTest) {
    try {
      const formBody = new URLSearchParams();
      if (xsrfToken) formBody.append('__RequestVerificationToken', xsrfToken);

      const res = await axios.post(`${baseUrl}/Marketing/TicketSourceList/?handler=${h}`, formBody.toString(), {
        headers: reqHeaders
      });
      console.log(`\nHandler ${h} status: ${res.status}`);
      const decomp = decompress(res.data);
      console.log(`Handler ${h} decompressed result:`, typeof decomp, Array.isArray(decomp) ? `Array length ${decomp.length}` : (typeof decomp === 'object' ? Object.keys(decomp) : String(decomp).substring(0, 200)));
      if (Array.isArray(decomp) && decomp.length > 0) {
        console.log(`Sample from ${h}:`, decomp[0]);
        await Bun.write(`scratch/ticket_source_${h}.json`, JSON.stringify(decomp, null, 2));
      }
    } catch (e: any) {
