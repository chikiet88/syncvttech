...
async function main() {
  ...
  console.log(`=== TIMONA STATS ===`);
  console.log(`Timona length: ${rowsTimona.length}`);
  console.log(`Timona_2406 length: ${rowsTimona2406.length}`);
  ...
  for (let i = 0; i < minLength; i++) {
    ...
    if ((r1[0] || '') !== (r2[0] || '')) {
      mismatches++;
      if (mismatches <= 10) {
        console.log(`  Row ${i + 1} code mismatch: Timona = "${r1[0]}", Timona_2406 = "${r2[0]}"`);
      }
    }
  }
