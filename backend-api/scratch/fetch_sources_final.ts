...
async function main() {
  ...
  // 3. GET /Marketing/TicketSourceList page to obtain page verification token
  ...
  if (decomp && (decomp.Table || decomp.Table1)) {
    ...
    console.log('Saved scratch/sources_decompressed.json');
    console.log(`Table (Nguồn chính): ${decomp.Table?.length || 0} items`);
    console.log(`Table1 (Nguồn chi tiết): ${decomp.Table1?.length || 0} items`);
    if (decomp.Table?.length > 0) {
      console.log('\nSample Nguồn chính (Table[0]):', decomp.Table[0]);
    }
    if (decomp.Table1?.length > 0) {
      console.log('\nSample Nguồn chi tiết (Table1[0]):', decomp.Table1[0]);
    }
  } else {
    console.log('Raw output:', String(res.data).substring(0, 500));
  }
}
