...
async function main() {
  ...
  // 1. Force the API service to ONLY use ittest1 (with full permissions)
  ...
  if (!ittest1Session) {
    console.error('❌ "ittest1" session not found! Please check your env variables.');
    await app.close();
    return;
  }
  ...
  // 2. Sync appointments for June 20 and June 21, 2026
  const targetDates = ['2026-06-20', '2026-06-21'];
  console.log(`⏳ Syncing appointments for ${targetDates.join(' and ')}...`);
  ...
  for (const dateStr of targetDates) {
    ...
    for (const branch of branches) {
      try {
        ...
        console.error(`   ❌ Error syncing branch ${branch.name} on ${dateStr}:`, e.message);
      }
    }
  }

  // 3. Find and repair customer name mismatches for June 20-21 appointments
  console.log('\n🔍 Finding customer name mismatches in database for June 20-21...');
  const mismatches = await prisma.$queryRawUnsafe(`
    ...
    WHERE a.customer_name <> c.name
      AND a.appointment_date >= '2026-06-20T00:00:00Z'
      AND a.appointment_date <= '2026-06-21T23:59:59Z'
  `) as any[];

  console.log(`Found ${mismatches.length} customer name mismatches.`);
