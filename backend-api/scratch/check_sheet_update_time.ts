    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        if (lastIndex >= 0) {
          ...
          // Print 3 rows before it too
          const start = Math.max(0, lastIndex - 3);
          console.log(`  Recent rows (from Row ${start + 1} to ${lastIndex + 1}):`);
          for (let i = start; i <= lastIndex; i++) {
            console.log(`    [Row ${i + 1}]`, rows[i]);
          }
        } else {
          console.log(`  Sheet is empty (only empty rows).`);
        }
      } else {
