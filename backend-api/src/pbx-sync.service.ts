...
export class PbxSyncService {
  ...
  async syncVttechCallHistory(dateFrom: string, dateTo: string) {
    ...
    try {
      ...
      for (const record of records) {
        ...
        try {
          ...
          if (statusStr.includes('Hoàn tất') || statusStr.toLowerCase().includes('answered')) callStatus = 'ANSWERED';
          else if (statusStr.includes('Gọi nhỡ') || statusStr.toLowerCase().includes('no answer')) callStatus = 'NO_ANSWER';
          else if (statusStr.includes('Bận') || statusStr.toLowerCase().includes('busy')) callStatus = 'BUSY';
          else if (statusStr.includes('Hủy')) callStatus = 'CANCELED';
