...
import { Scissors, Download, Calendar as CalendarIcon, Loader2, Search, ArrowLeft } from "lucide-react"
...
function ServicesReportContent() {
  ...
  const [data, setData] = useState<ServiceColumn[]>([])
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      url.searchParams.set("branchID", branchID) // Backend uses branchID
      url.searchParams.set("dateFrom", formatDate(dateFrom)) // Backend uses dateFrom
      url.searchParams.set("dateTo", formatDate(dateTo)) // Backend uses dateTo
      url.searchParams.set("service_only", "true") // The filter we added
    ...
    <div className="p-6 space-y-6 animate-fade-in">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            <h2 className="text-2xl font-bold tracking-tight flex items-center gap-3">
              ...
              Chi tiết Dịch vụ Bán ra
            </h2>
            <p className="text-sm text-muted-foreground">
              Tra cứu danh sách các dịch vụ phát sinh trong hóa đơn theo chi nhánh.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div className="space-y-2">
              ...
              <Select value={branchID} onValueChange={setBranchID}>
                <SelectTrigger className="rounded-xl border-none bg-slate-100 h-11 focus:ring-1 focus:ring-blue-500 text-black shadow-inner shadow-slate-200">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-9 rounded-xl border-none bg-slate-100 h-11 focus:ring-1 focus:ring-blue-500 text-black shadow-inner shadow-slate-200"
                />
              </div>
            </div>
            ...
            <Button 
              className="h-11 rounded-xl font-bold bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-500/20 gap-2 transition-all active:scale-95"
      ...
      <Card className="glass border-none shadow-xl rounded-2xl overflow-hidden mt-6">
        <CardHeader className="p-6 pb-2 border-b border-slate-50">
          <CardTitle className="text-lg">Dữ liệu Dịch vụ</CardTitle>
          <CardDescription className="text-xs">Trực tiếp từ hoá đơn tài chính đã được thanh toán hoặc phát sinh giao dịch.</CardDescription>
        </CardHeader>
...
export default function ServicesReportPage() {
  return (
    <Suspense fallback={<div className="p-6 flex justify-center mt-20"><Loader2 className="w-8 h-8 animate-spin text-blue-500" /></div>}>
