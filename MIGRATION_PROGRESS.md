...
6. [x] **Mới:** Triển khai cơ chế **Smart Fallback** trong `manage.sh` (Tự động chuyển từ port 5432 nội bộ sang 18103 nếu chạy trên Host).
7. [x] Cập nhật root `package.json` với các tùy chọn `:host` cho lệnh `bun`.

## 🚀 Ghi chú về kết nối
- **Cơ chế tự động**: Lệnh `bun dev` hoặc các tùy chọn trong `manage.sh` sẽ tự động phát hiện môi trường. Nếu không tìm thấy host `tazagroupnet-db`, hệ thống sẽ tự dùng `localhost:18103`.
